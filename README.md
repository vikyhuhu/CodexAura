...
3. 点主题缩略图即可换肤；拖动「暗角 / 模糊 / 蒙版」滑杆实时微调
...
图片内置预设包含「葫芦娃」「元气食堂」「蓝调缪斯」「琥珀黄昏」「赛博霓虹」「森野薄雾」「午夜极光」「樱粉晨曦」；基础渐变模板包含「极光」「落日」「深海」。图片内置预设会随版本升级，不能删除；仍可导出分享，暗角、模糊和蒙版微调也会在升级时保留。升级时已下架的旧内置预设会自动清理，但同名 ID 的用户自建主题不会被误删。
...
```json
{
  ...
  "colors": { "background": "#101216", "panel": "#171a20", "accent": "#7aa2f7",
              ...
              "onAccent": "#000000" },
  "dim": 0.35, "blur": 0, "contentMask": 1, "focusX": 0.5, "focusY": 0.45
}
```

`appearance` 可设为 `auto`、`light` 或 `dark`；省略时自动根据背景色选择控件外观和遮罩方向。`contentMask` 控制内容区可读性渐变强度（`0...1`），省略时保持完整强度。`onAccent` 省略时会在黑白之间自动选择对比度更高的按钮文字色。旧主题无需迁移，缺少这些字段时仍按兼容默认值读取。
...
单元测试覆盖旧主题兼容、明暗主题推导、强调色文字对比度、内置预设安装/升级/幂等、下架预设安全清理、删除保护，以及全部内置图片的尺寸和体积约束。

项目采用“功能分支 → PR → CI → 产品验收 → 合并”的协作方式。CI 会自动运行单元测试、双架构构建、签名和资源检查；完整说明见 [开发与发布流程](docs/DEVELOPMENT_WORKFLOW.md)。
